...
if !(dist == Normal || dist == Categorical)
    error("Unsupported dist=$(dist). Continuous1 supports Normal and Categorical.")
end
        ...
        action = if make_deterministic
            if dist == Categorical
                ...
                if multi_agent
                    ...
                    @assert length(action_dist) == 1 "Categorical Continuous1 expects one discrete action head."
                    mode(action_dist[1])
                end
            else
