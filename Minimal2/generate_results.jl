...
algorithms = [
    ("SAC", "Minimal2/SAC/Minimal2_SAC.jl"),
    ("SAC2", "Minimal2/SAC2/Minimal2_SAC2.jl"),
    ("PPO", "Minimal2/PPO/Minimal2_PPO.jl"),
    ("PPO3", "Minimal2/PPO3/Minimal2_PPO3.jl"),
    ("DDPG", "Minimal2/DDPG/Minimal2_DDPG.jl")
]
...
#     ("SAC2", "Minimal2/SAC2/Minimal2_SAC2.jl"),
#     ("PPO", "Minimal2/PPO/Minimal2_PPO.jl"),
#     ("PPO2", "Minimal2/PPO2/Minimal2_PPO2.jl"),
#     ("PPO3", "Minimal2/PPO3/Minimal2_PPO3.jl"),
#     ("DDPG", "Minimal2/DDPG/Minimal2_DDPG.jl")
# ]
...
test_set_file = joinpath(@__DIR__, "test_set.jld2")

function load_test_set(file_path::String)
    loaded = FileIO.load(file_path)
    if haskey(loaded, "test_set")
        return loaded["test_set"]
    elseif haskey(loaded, "set")
        test_set = loaded["set"]
        FileIO.save(file_path, "test_set", test_set)
        return test_set
    else
        error("No test set found in $file_path")
    end
end

function generate_test_set(; n = 200)
    global test_set = []

    for _ in 1:n
        wind = generate_wind()
        gp = generate_grid_price()
        push!(test_set, (wind, gp))
    end

    return test_set
end

function ensure_test_set!(; n = 200)
    if @isdefined(test_set) && !isnothing(test_set) && !isempty(test_set)
        return test_set
    end

    if isfile(test_set_file)
        global test_set = load_test_set(test_set_file)
        return test_set
    end

    if !(@isdefined(generate_wind) && @isdefined(generate_grid_price))
        error("test_set.jld2 not found and generators are unavailable. Load an environment script first.")
    end

    global test_set = generate_test_set(n = n)
    FileIO.save(test_set_file, "test_set", test_set)
    println("Generated and saved new test set to $test_set_file")
    return test_set
end

if isfile(test_set_file)
    global test_set = load_test_set(test_set_file)
    println("Loaded existing test set file")
elseif @isdefined(generate_wind) && @isdefined(generate_grid_price)
    global test_set = generate_test_set()
    FileIO.save(test_set_file, "test_set", test_set)
    println("Generated and saved new test set file")
else
    global test_set = nothing
    println("test_set.jld2 not found. Will generate it on first call to test_agent/plot_test_comparison.")
end
        ...
        for il_type in il_types
            for rs_type in rs_types
                ...
                for i in 1:n
                    ...
                    #collect_runs(1; selected_algorithms = ["PPO"], il_types = ["IL", "no_IL"], rs_types = ["with_RS"])
...
function test_agent(; optimizer = false, render = false)
    global test_rewards = Float32[]
    xx = collect(dt / 60:dt / 60:te / 60)
    test_set_local = ensure_test_set!()

    for (w, gp) in test_set_local
        reset!(env)
        global wind = [w]
        ...
        if optimizer
            ...
            while !(env.terminated || env.truncated)
                ...
                if render
                    push!(results_run["hpc"], clamp((action[1] + 1) * 0.5, 0, 1))
...
function plot_test_comparison(; current = false)
    ...
    optimal_scores = if haskey(results, "Optimal") &&
                       ...
                       haskey(results["Optimal"]["baseline"], "test_scores") &&
                       haskey(results["Optimal"]["baseline"]["test_scores"], 1)
        println("Using cached optimal test baseline scores from results...")
        results["Optimal"]["baseline"]["test_scores"][1]["data"]
    else
        println("Computing optimal test baseline scores...")
        global optimal_scores = test_agent(optimizer = true)
        ...
        if !haskey(results["Optimal"]["baseline"], "test_scores")
            results["Optimal"]["baseline"]["test_scores"] = Dict{Int, Dict{String, Any}}()
        end
        results["Optimal"]["baseline"]["test_scores"][1] = Dict{String, Any}("data" => optimal_scores)
        FileIO.save(results_file, "results", results)
        println("Saved optimal test baseline scores to results file")
        optimal_scores
    end

    untrained_scores = if haskey(results, "Untrained") &&
                         haskey(results["Untrained"], "baseline") &&
                         haskey(results["Untrained"]["baseline"], "test_scores") &&
                         haskey(results["Untrained"]["baseline"]["test_scores"], 1)
        println("Using cached untrained test baseline scores from results...")
        results["Untrained"]["baseline"]["test_scores"][1]["data"]
    else
        println("Computing untrained test baseline scores...")

        had_agent = @isdefined(agent)
        had_agent_save = @isdefined(agent_save)
        agent_backup = had_agent ? deepcopy(agent) : nothing
        agent_save_backup = had_agent_save ? deepcopy(agent_save) : nothing

        include("Minimal2/SAC2/Minimal2_SAC2.jl")
        computed_untrained_scores = test_agent()

        if had_agent
            global agent = agent_backup
        elseif @isdefined(agent)
            global agent = nothing
        end

        if had_agent_save
            global agent_save = agent_save_backup
        elseif @isdefined(agent_save)
            global agent_save = nothing
        end

        if !haskey(results, "Untrained")
            results["Untrained"] = Dict{String, Dict{String, Dict{Int, Dict{String, Any}}}}()
        end
        if !haskey(results["Untrained"], "baseline")
            results["Untrained"]["baseline"] = Dict{String, Dict{Int, Dict{String, Any}}}()
        end
        if !haskey(results["Untrained"]["baseline"], "test_scores")
            results["Untrained"]["baseline"]["test_scores"] = Dict{Int, Dict{String, Any}}()
        end
        results["Untrained"]["baseline"]["test_scores"][1] = Dict{String, Any}("data" => computed_untrained_scores)
        FileIO.save(results_file, "results", results)
        println("Saved untrained test baseline scores to results file")
        computed_untrained_scores
    end

    global best_test_results = Dict{String, Any}()
    best_test_results["Optimal"] = (optimal_scores, nothing, nothing)
    best_test_results["Untrained"] = (untrained_scores, nothing, nothing)

    for alg_name in keys(results)
        (alg_name == "Optimal" || alg_name == "Untrained") && continue
        ...
        for il_type in keys(results[alg_name])
            for rs_type in keys(results[alg_name][il_type])
                ...
                for seed in keys(results[alg_name][il_type][rs_type])
                    ...
                    println("Testing $(alg_name)-$(il_type)-$(rs_type)-seed$(seed)...")
                    scores = test_agent()
    ...
    if current
        if !isnothing(agent_save)
            ...
            current_scores = test_agent()

            best_test_results["Current"] = (
                current_scores,
                validation_scores,
                nothing
            )
    ...
    order = sort(collect(keys(best_test_results)),
                 by = key -> mean(best_test_results[key][1]),
                 rev = true)

    color_map = Dict{String, Vector{Int}}()
    color_map["Optimal"] = [76, 175, 80]
    color_map["Untrained"] = [239, 83, 80]
    color_map["Current"] = [239, 83, 239]
    ...
    for key in order
        scores, _, best_seeds = best_test_results[key]
    ...
    layout1 = Layout(
        title = "Algorithm Test Performance Comparison",
    ...
    for key in order
        _, timeline, best_seeds = best_test_results[key]
...
function plot_validation_comparison(; current = false)
    ...
    include("Minimal2/Validation_Minimal2.jl")
...
# include("Minimal2/SAC2/Minimal2_SAC2.jl")
# include("Minimal2/generate_results.jl")
# collect_runs(
#            1;
#            selected_algorithms = ["PPO", "SAC2", "SAC", "DDPG"],
#            il_types = ["no_IL", "IL"],
#            rs_types = ["no_RS"],
#        )
