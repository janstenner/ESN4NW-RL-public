...
const modelbased_fm_season = "Winter"
const modelbased_fm_serial = "1011089"
const modelbased_fm_steps = 38
const modelbased_stats_path = joinpath(@__DIR__, "models", "HK_blocks", "stats_by_season_serial.json")

global modelbased_fm_model = nothing
global modelbased_common_one = nothing

@inline function _modelbased_feature_index(name::AbstractString)::Int
    idx = findfirst(==(name), ORDERED_BASE)
    isnothing(idx) && error("Feature $name not found in ORDERED_BASE.")
    return idx
end

function _modelbased_get_fm_model()
    global modelbased_fm_model
    if isnothing(modelbased_fm_model)
        modelbased_fm_model = load_fm_model(; serial = modelbased_fm_serial, assign_globals = false)
    end
    return modelbased_fm_model
end

function _modelbased_get_common_one(; verbose = false)
    global modelbased_common_one
    if !isnothing(modelbased_common_one)
        return modelbased_common_one
    end

    stats = JSON.parsefile(modelbased_stats_path)
    season_list = ("Winter", "Fruehling", "Sommer", "Herbst")
    season_common_ones = Float64[]

    verbose && println("ModelBased common_one per season:")
    for season in season_list
        haskey(stats, season) || error("Season '$season' missing in $modelbased_stats_path")
        haskey(stats[season], modelbased_fm_serial) || error("Serial '$modelbased_fm_serial' missing for season '$season' in $modelbased_stats_path")

        season_stats = stats[season][modelbased_fm_serial]
    ...
    modelbased_common_one = Float32(maximum(season_common_ones))
    verbose && println("ModelBased shared common_one (all seasons): $(modelbased_common_one)")
    return modelbased_common_one
end
...
function _generate_modelbased_signals(; season::AbstractString = modelbased_fm_season)
    global history_steps, te, dt

    dt == 5.0 || error("ModelBased FM signal generator currently expects dt=5.0, got dt=$(dt).")
    ...
    model = _modelbased_get_fm_model()
    seed = rand(1:typemax(Int32))
    pred_z = generate_slots_z(model;
                              n_slots = total_slots_10,
                              season = season,
                              day_start = day_start,
                              steps = modelbased_fm_steps,
                              seed = seed)
    pred_denorm = inverse_log1p_zscore(pred_z; serial = modelbased_fm_serial)

    idx_tech = _modelbased_feature_index("power_avail_tech_mean_kw")
    idx_force = _modelbased_feature_index("power_avail_force_maj_mean_kw")
    idx_ext = _modelbased_feature_index("power_avail_ext_mean_kw")
    ...
    common_one = _modelbased_get_common_one()
...
@inline function _modelbased_season_tag(season::AbstractString)
    return replace(strip(String(season)), r"[^A-Za-z0-9_-]" => "_")
end

function modelbased_generated_set_path(; season::AbstractString = modelbased_fm_season)
    return joinpath(@__DIR__, "modelbased_generated_wind_set_$(_modelbased_season_tag(season)).jld2")
end
...
function _load_generated_wind_set(; season::AbstractString = modelbased_fm_season,
                                   path::Union{Nothing,AbstractString} = nothing)
    path_final = isnothing(path) ? modelbased_generated_set_path(; season = season) : String(path)
...
function generate_wind_set(; n::Int = 2000,
                           season::AbstractString = modelbased_fm_season,
                           path::Union{Nothing,AbstractString} = nothing,
    ...
    for i in 1:n
        generated[i] = _generate_modelbased_signals(; season = season)
        next!(progress)
    end
    ...
    global modelbased_generated_wind_set_season = String(season)
    global modelbased_generated_wind_set = generated
    return generated
end

# Default load of pre-generated wind/curtailment pairs for the configured season.
global modelbased_generated_wind_set_season = String(modelbased_fm_season)
global modelbased_generated_wind_set = _load_generated_wind_set(; season = modelbased_generated_wind_set_season)
...
function generate_wind(; same_day = false, use_generated = true)
    ...
    if use_generated
        if modelbased_generated_wind_set_season != modelbased_fm_season
            global modelbased_generated_wind_set_season = String(modelbased_fm_season)
            global modelbased_generated_wind_set = _load_generated_wind_set(; season = modelbased_generated_wind_set_season)
        end

        if !isempty(modelbased_generated_wind_set)
            w, ct = rand(modelbased_generated_wind_set)
            return deepcopy(w), deepcopy(ct)
        end
    end
    return _generate_modelbased_signals(; season = modelbased_fm_season)
end

function generate_grid_price(; same_day = false)
    global history_steps
    return sample_grid_price_day(;
        season = modelbased_fm_season,
...
include("./Validation_ModelBased.jl")
...
function calculate_day(action, env, step = nothing; reward_shaping = reward_shaping)
    ...
    for i in 1:n_windCORES
        compute_power += action[i]*0.0075/n_windCORES
    end
