...
function plot_trajectory()
    ...
    push!(to_plot, scatter(y=(t[:action][:] .+ 1.0) .* 0.5, name="WindCORE utilization 1"))

    push!(to_plot, scatter(y=states[6,:], name="Wind Power 1"))

    push!(to_plot, scatter(y=Float32.(t[:terminated][:]), name="Terminal"))
