...
# Curtailment threshold is now a generated time series (same length as wind/grid).
curtailment_threshold_default = 0.4f0
curtailment_threshold = fill(curtailment_threshold_default, Int(te/dt) + history_steps)

include("./models/FM_day_runtime.jl")
include("./models/grid_prices/grid_price_loader.jl")

const modelbased1_fm_season = "Winter"
const modelbased1_fm_serial = "1011089"
const modelbased1_fm_steps = 38
const modelbased1_stats_path = joinpath(@__DIR__, "models", "HK_blocks", "stats_by_season_serial.json")

global modelbased1_fm_model = nothing
global modelbased1_common_one = nothing

@inline function _modelbased1_feature_index(name::AbstractString)::Int
    idx = findfirst(==(name), ORDERED_BASE)
    isnothing(idx) && error("Feature $name not found in ORDERED_BASE.")
    return idx
end

function _modelbased1_get_fm_model()
    global modelbased1_fm_model
    if isnothing(modelbased1_fm_model)
        modelbased1_fm_model = load_fm_model(; serial = modelbased1_fm_serial, assign_globals = false)
    end
    return modelbased1_fm_model
end

function _modelbased1_get_common_one(; verbose = false)
    global modelbased1_common_one
    if !isnothing(modelbased1_common_one)
        return modelbased1_common_one
    end

    stats = JSON.parsefile(modelbased1_stats_path)
    season_list = ("Winter", "Fruehling", "Sommer", "Herbst")
    season_common_ones = Float64[]

    verbose && println("ModelBased1 common_one per season:")
    for season in season_list
        haskey(stats, season) || error("Season '$season' missing in $modelbased1_stats_path")
        haskey(stats[season], modelbased1_fm_serial) || error("Serial '$modelbased1_fm_serial' missing for season '$season' in $modelbased1_stats_path")

        season_stats = stats[season][modelbased1_fm_serial]
        tech = season_stats["power_avail_tech_mean_kw"]
        force = season_stats["power_avail_force_maj_mean_kw"]

        tech_one = max(Float64(tech["mu"]) + 3.0 * Float64(tech["sigma"]), 1e-6)
        force_one = max(Float64(force["mu"]) + 3.0 * Float64(force["sigma"]), 1e-6)
        season_common_one = min(tech_one, force_one)
        push!(season_common_ones, season_common_one)
        verbose && println("  $season: $season_common_one")
    end

    # Use one shared factor across all seasons.
    modelbased1_common_one = Float32(maximum(season_common_ones))
    verbose && println("ModelBased1 shared common_one (all seasons): $(modelbased1_common_one)")
    return modelbased1_common_one
end

function _interpolate_10min_to_5min(y10::AbstractVector{<:Real})
    n10 = length(y10)
    y5 = Vector{Float32}(undef, 2 * n10)

    @inbounds for k in 1:length(y5)
        if isodd(k)
            y5[k] = Float32(y10[(k + 1) ÷ 2])
        else
            idx = k ÷ 2
            if idx < n10
                y5[k] = 0.5f0 * (Float32(y10[idx]) + Float32(y10[idx + 1]))
            else
                y5[k] = Float32(y10[end])
            end
        end
    end
...
function _generate_modelbased1_signals(; season::AbstractString = modelbased1_fm_season)
    global history_steps, te, dt

    dt == 5.0 || error("ModelBased1 FM signal generator currently expects dt=5.0, got dt=$(dt).")

    main_steps = Int(round(te / dt))
    target_len = main_steps + history_steps

    # Generate an extended 10-minute sequence so 5-minute interpolation includes history before 00:00.
    lead_slots_10 = Int(cld(history_steps + 1, 2))
    total_slots_10 = SLOTS + lead_slots_10
    start_slot = mod1(1 - lead_slots_10, SLOTS)
    start_minutes = (start_slot - 1) * 10
    day_start = Time(Int(fld(start_minutes, 60)), Int(mod(start_minutes, 60)))

    model = _modelbased1_get_fm_model()
    seed = rand(1:typemax(Int32))
    pred_z = generate_slots_z(model;
                              n_slots = total_slots_10,
                              season = season,
                              day_start = day_start,
                              steps = modelbased1_fm_steps,
                              seed = seed)
    pred_denorm = inverse_log1p_zscore(pred_z; serial = modelbased1_fm_serial)

    idx_tech = _modelbased1_feature_index("power_avail_tech_mean_kw")
    idx_force = _modelbased1_feature_index("power_avail_force_maj_mean_kw")
    idx_ext = _modelbased1_feature_index("power_avail_ext_mean_kw")

    tech = vec(pred_denorm[idx_tech, :])
    force = vec(pred_denorm[idx_force, :])
    ext = vec(pred_denorm[idx_ext, :])

    wind_power_10 = min.(tech, force)
    curtailment_threshold_10 = ext

    common_one = _modelbased1_get_common_one()
    wind_power_10 = Float32.(wind_power_10 ./ common_one)
    curtailment_threshold_10 = Float32.(curtailment_threshold_10 ./ common_one)

    wind_power_5 = _interpolate_10min_to_5min(wind_power_10)
    curtailment_threshold_5 = _interpolate_10min_to_5min(curtailment_threshold_10)

    # Align so index history_steps + 1 corresponds to 00:00 for history_steps=5.
    wind_power_5 = wind_power_5[2:end]
    curtailment_threshold_5 = curtailment_threshold_5[2:end]

    if length(wind_power_5) < target_len
        error("Generated wind signal too short: $(length(wind_power_5)) < $target_len")
    end

    return wind_power_5[1:target_len], curtailment_threshold_5[1:target_len]
end

@inline function _modelbased1_season_tag(season::AbstractString)
    return replace(strip(String(season)), r"[^A-Za-z0-9_-]" => "_")
end

function modelbased1_generated_set_path(; season::AbstractString = modelbased1_fm_season)
    return joinpath(@__DIR__, "modelbased1_generated_wind_set_$(_modelbased1_season_tag(season)).jld2")
end

function _normalize_generated_wind_entry(entry)
    if entry isa Tuple && length(entry) == 2
        w, ct = entry
        return (Float32.(w), Float32.(ct))
    end
    error("Unsupported generated wind entry format: $(typeof(entry))")
end

function _load_generated_wind_set(; season::AbstractString = modelbased1_fm_season,
                                   path::Union{Nothing,AbstractString} = nothing)
    path_final = isnothing(path) ? modelbased1_generated_set_path(; season = season) : String(path)
    if !isfile(path_final)
        return Tuple{Vector{Float32}, Vector{Float32}}[]
    end
    loaded = FileIO.load(path_final, "set")
    return [_normalize_generated_wind_entry(e) for e in loaded]
end

function generate_wind_set(; n::Int = 2000,
                           season::AbstractString = modelbased1_fm_season,
                           path::Union{Nothing,AbstractString} = nothing,
                           save_to_file::Bool = true)
    n > 0 || error("n must be > 0, got $n")
    path_final = isnothing(path) ? modelbased1_generated_set_path(; season = season) : String(path)

    generated = Vector{Tuple{Vector{Float32}, Vector{Float32}}}(undef, n)
    progress = Progress(n, 1)
    ProgressMeter.update!(progress, 1)

    for i in 1:n
        generated[i] = _generate_modelbased1_signals(; season = season)
        next!(progress)
    end

    if save_to_file
        FileIO.save(path_final, "set", generated)
    end

    global modelbased1_generated_wind_set_season = String(season)
    global modelbased1_generated_wind_set = generated
    return generated
end

# Default load of pre-generated wind/curtailment pairs for the configured season.
global modelbased1_generated_wind_set_season = String(modelbased1_fm_season)
global modelbased1_generated_wind_set = _load_generated_wind_set(; season = modelbased1_generated_wind_set_season)

function generate_wind(; same_day = false, use_generated = true)
    # same_day is intentionally ignored for FM-based generation (season fixed, seed randomized).
    if use_generated
        if modelbased1_generated_wind_set_season != modelbased1_fm_season
            global modelbased1_generated_wind_set_season = String(modelbased1_fm_season)
            global modelbased1_generated_wind_set = _load_generated_wind_set(; season = modelbased1_generated_wind_set_season)
        end

        if !isempty(modelbased1_generated_wind_set)
            w, ct = rand(modelbased1_generated_wind_set)
            return deepcopy(w), deepcopy(ct)
        end
    end
    return _generate_modelbased1_signals(; season = modelbased1_fm_season)
end

function generate_grid_price(; same_day = false)
    global history_steps
    return sample_grid_price_day(;
        season = modelbased1_fm_season,
...
function create_state(; env = nothing, compute_left = 1.0, step = 0, generate_day = true, same_day = false)
    ...
    if isnothing(env)
        ...
        if generate_day
            wind_signal, ct_signal = generate_wind(; same_day = same_day)
            wind = [deepcopy(wind_signal) for _ in 1:n_turbines]
            curtailment_threshold = deepcopy(ct_signal)
    ...
    current_curtailment_threshold = curtailment_threshold[history_steps+step]
    push!(y, current_curtailment_threshold)
    ...
    for i in 1:n_turbines
        ...
        push!(y, max(0.0, wind[i][history_steps+step] - current_curtailment_threshold))
    end
...
include("./Validation_ModelBased1.jl")
...
function calculate_day(action, env, step = nothing; reward_shaping = reward_shaping)
    ...
    step += history_steps
    current_curtailment_threshold = curtailment_threshold[step-1]
    ...
    if wind_only
        ...
        for i in 1:n_turbines
            ...
            temp_free_power = (wind[i][step-1] - current_curtailment_threshold)
...
function render_run_shared(;
    ...
    if ddpg_mode
        ...
        push!(to_plot, scatter(y = curtailment_threshold, name = "Curtailment Threshold"))
        for k in 1:n_turbines
            push!(to_plot, scatter(y = results_run["hpc$k"], name = "WindCORE utilization $k"))
            push!(to_plot, scatter(y = wind[k], name = "Wind Power $k"))
            free_energy = max.(0.0, wind[k] .- curtailment_threshold)
            push!(to_plot, scatter(y = free_energy, name = "Free Energy $k"))
        end
    else
        if show_σ
            for k in 1:n_windCORES
        ...
        push!(to_plot, scatter(x = xx, y = curtailment_threshold[history_steps:end], name = "Curtailment Threshold"))
        ...
        for k in 1:n_turbines
            ...
            free_energy = max.(0.0, wind[k][history_steps:end] .- curtailment_threshold[history_steps:end])
            push!(to_plot, scatter(x = xx, y = free_energy, name = "Free Energy $k"))
        end
    end
    ...
    if plot_critic2 && profile != :ppo
        ...
        if critic2_diagnostics
            ...
            temp_state[2] = 1.0f0
            ct_index = 2 + include_history_steps + include_gradients
            ...
            temp_state[end-1] = clamp(temp_state[6] - temp_state[ct_index], 0.0f0, 1.0f0)
            push!(new_states, deepcopy(temp_state))

            xx_diag = Float32[0.0f0]
            for _ in 1:288
                temp_state[wind_index] += 1.0f0 / 288.0f0
                temp_state[end-1] = clamp(temp_state[wind_index] - temp_state[ct_index], 0.0f0, 1.0f0)
                push!(new_states, deepcopy(temp_state))
